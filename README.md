...
Малък уеб проект (SWP): PHP backend + JavaScript на клиента за показване на **текуща/последна F1 сесия** — класиране, интервали (при състезание) и скорошни съобщения от Race Control; страници за **календар на сезона** (уикенди и сесии) и **генерално класиране** (пилоти и конструктори). Данните идват от публичния **OpenF1** API.
...
Страницата `calendar.php` показва Grand Prix уикенди с флаг, локация, период на събитието и таблица с всички сесии (Practice, Qualifying, Sprint, Race и др.) с локални часове.

![F1 календар — Miami, Canada, Monaco и др.](./docs/screenshot-calendar.png)

### Генерално класиране

Страницата `standings.php` показва **пилоти** и **конструктори** с точки и позиции спрямо данните от OpenF1 (`championship_drivers` / `championship_teams`), с контекст за сесията (напр. Race · Suzuka · Japan).

![F1 генерално класиране — пилоти и конструктори](./docs/screenshot-standings.png)
...
- **Какво се ползва в проекта:**
  - `api/live.php` — към `sessions`, `drivers`, `intervals`, `position`, `race_control` (с `session_key=latest` където е подходящо).
  - `api/calendar.php` — към `meetings` и `sessions` за избрана година (календар на сезона).
  - `api/standings.php` — към `championship_drivers`, `championship_teams`, `drivers`, `sessions` (генерално класиране; бета крайни точки в OpenF1).
...
├── calendar.php        # Календар по сезон (UI)
├── standings.php       # Генерално класиране — пилоти и конструктори (UI)
├── api/
│   ├── live.php        # JSON — живо табло
│   ├── calendar.php    # JSON — срещи и сесии по година
│   └── standings.php   # JSON — класиране (OpenF1 championship_*)
├── css/
├── includes/
├── docs/
│   ├── screenshot.png           # README — живо табло
│   ├── screenshot-calendar.png  # README — календар
│   └── screenshot-standings.png # README — генерално класиране
└── README.md
```
...
| Къде е календарът? | Страница **Календар** (`calendar.php`) — сезони от 2023 нагоре, данни от `meetings` + `sessions`. |
| Къде е генералното класиране? | Страница **Класиране** (`standings.php`), данни през `api/standings.php` (OpenF1 championship). |
| Нужен ли е API ключ? | За текущата интеграция — не (виж документацията на OpenF1 за бъдещи промени). |
| Колко често се обновява? | Живото табло: приблизително на всеки **5 секунди**. Календарът — при смяна на сезона (кеш ~1 ч.). Класирането — при зареждане на страницата (кеш на API отговор ~5 мин.). |
...
| `/calendar.php` | Календар на сезона |
| `/standings.php` | Генерално класиране — пилоти и конструктори |
| `/privacy.php`, `/terms.php`, `/faq.php`, `/sitemap.php` | Поверителност, условия, ЧЗВ, карта на сайта |
| `/api/live.php` | JSON за живото табло (AJAX) |
| `/api/calendar.php` | JSON за календар (`?year=2026`) |
| `/api/standings.php` | JSON за генерално класиране |
